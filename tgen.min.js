...
1],g=Math.round(e.percent/100*this.size),f=Math.round(h.percent/100*this.size),n=g;n<=f;n++)c[b?this.size-n:n]=[e.rgba[0]+(n-g)/(f-g)*(h.rgba[0]-e.rgba[0]),e.rgba[1]+(n-g)/(f-g)*(h.rgba[1]-e.rgba[1]),e.rgba[2]+(n-g)/(f-g)*(h.rgba[2]-e.rgba[2]),e.rgba[3]+(n-g)/(f-g)*(h.rgba[3]-e.rgba[3])];return c},get(a,b){if(void 0==a)return this.data;a=e.calc.pingpong(parseInt(a),0,255);a>this.size&&(a-=this.size);0>a&&(a+=this.size);var c=this.data[a];void 0!==b&&(c[3]=b[3]);return c}};e.wrapx=(a)=>a&c-1;e.wrapy=
(b)=>b&a-1;e.point={rgba:[],mixed:[],blend:"opacity",colorize(a,b,c){void 0===c&&(c=50);return[a[0]-c/100*(a[0]-b[0]),a[1]-c/100*(a[1]-b[1]),a[2]-c/100*(a[2]-b[2]),b[3]?b[3]:255]},opacity(a,b){if(255==a[3])return a[3]=b[3],a;if(0==b[3])return b;var c=a[3]/255;return[a[0]*c+b[0]*(1-c),a[1]*c+b[1]*(1-c),a[2]*c+b[2]*(1-c),b[3]]},set(a,b){var c=e.texture.get(a,b);void 0!==d.blends[this.blend]?(this.rgba=d.blends[this.blend](e,c,this.rgba),this.mixed=this.opacity(this.rgba,c)):this.mixed=this.rgba;switch(e.normalize){case "clamped":this.mixed[0]=
Math.max(0,Math.min(255,this.mixed[0]));this.mixed[1]=Math.max(0,Math.min(255,this.mixed[1]));this.mixed[2]=Math.max(0,Math.min(255,this.mixed[2]));this.mixed[3]=Math.max(0,Math.min(255,this.mixed[3]));break;case "pingpong":this.mixed[0]=e.calc.pingpong(this.mixed[0],0,255),this.mixed[1]=e.calc.pingpong(this.mixed[1],0,255),this.mixed[2]=e.calc.pingpong(this.mixed[2],0,255),this.mixed[3]=e.calc.pingpong(this.mixed[3],0,255)}e.texture.set(a,b,this.mixed)},get(a,b,c){return e.texture.get(a,b)}};e.walk=
(b)=>{for(var d=0;d<c;d++)for(var h=0;h<a;h++){var g=e.point.get(d,h),g=b(g,d,h);e.point.rgba=g;e.point.set(d,h)}};e.percent=(a,b)=>parseInt(a/100*b,10);e.percentX=(a)=>parseInt(a/100*c,10);e.percentY=(b)=>parseInt(b/100*a,10);e.percentXY=(a)=>parseInt(a/100*f,10);e.xysize=(b,d)=>{if(void 0!=d.elements)var h=d.elements[b].x,g=d.elements[b].y,f=d.elements[b].size;else"random"==d.origin?(h=e.randIntSeed(0,c),g=e.randIntSeed(0,a)):(h=d.origin[0],g=d.origin[1]),f=e.randByArraySeed(d.size);return{x:h,
y:g,size:f}};e.toContext=(b,d)=>{for(var e=b.createImageData(c,a),h=e.data,g=d.length,f=0;f<g;f+=4)h[f]=d[f],h[f+1]=d[f+1],h[f+2]=d[f+2],h[f+3]=d[f+3];return e};e.toCanvas=function(b){void 0==b&&(b=e.texture.data);var d=document.createElement("canvas");d.width=c;d.height=a;var h=d.getContext("2d");b=this.toContext(h,b);h.putImageData(b,0,0);return d};e.getCanvas=function(a){if(a){var b=this.toCanvas(this.layers[this.layers.length-1]);a(b)}return this};e.getPhases=function(a){if(a){for(var b=[],c=
e.layers.length,d=0;d<c;d++)b.push(this.toCanvas(this.layers[d]));a(b)}return this};e.stat=function(a){g.stop=(new Date).getTime();g.elapsed=(g.stop-g.start)/1E3;a&&a(g);return this};e.history={available(){try{return"localStorage"in window&&null!==window.localStorage&&void 0!==window.localStorage}catch(a){return!1}},list(){return this.available()?JSON.parse(window.localStorage.getItem(d.config.historyName))||[]:[]},last(){return this.available()?d.config.historyList[d.config.historyList.length-1]:
[]},get(a){return this.available()?d.config.historyList[a]?d.config.historyList[a]:null:[]},add(a){if(d.config.historyLast){if(!this.available())return[];a=e.params(a);d.config.historyList=JSON.parse(window.localStorage.getItem(d.config.historyName))||[];d.config.historyList.length>=d.config.historyLast&&d.config.historyList.shift();d.config.historyList.push(a);window.localStorage.setItem(d.config.historyName,JSON.stringify(d.config.historyList))}}};e.params=(b)=>{void 0==b&&(b=new Date,b=b.getHours()+
":"+b.getMinutes()+":"+b.getSeconds()+" l"+e.layers.length+" i"+h.length);return{name:b,version:d.version,width:c,height:a,normalize:e.normalize,items:h}};e.render=function(b,h){e.event("beforeRender",b);var g=0;void 0!=b.width&&(c=b.width);void 0!=b.height&&(a=b.height);e.normalize=void 0!=b.normalize?b.normalize:"limitless";k();e.texture=new e.buffer(b.background);1!=h&&e.clear();for(var f in b.items){n=b.items[f][0];var p=b.items[f][1],l=b.items[f][2];"random"==p&&(p=q(e.defaults));"object"==typeof p&&
(p=e.randItem(p));g!=n&&(void 0!=e.layers[n]?e.texture.data=e.layers[n]:e.texture.clear(),g=n);if(void 0!=e[p])e[p](l);else void 0!=d.effects[p]?e.do(p,l):console.warn("undefined effect: "+p);e.layers[n]=e.texture.export()}e.event("afterRender",e.params());e.history.add();return this};e.event=(a,b)=>{for(var c in d.events[a]){var h=d.events[a][c];"function"==typeof h&&h(e,b)}};e.do=(a,b)=>{var c=b;void 0===b?b=m({},d.defaults[a]):"object"==typeof b&&(b=m(d.defaults[a],b));var g=b;"object"==typeof g.count&&
(g.count=e.randInt(g.count[0],g.count[1]));"random"===g.blend&&(g.blend=q(d.blends));"object"==typeof g.blend&&(g.blend=g.blend[e.randInt(0,g.blend.length-1)]);e.point.blend=void 0!==g.blend?g.blend:"";g.rgba&&(g.rgba=e.rgba(g.rgba),e.point.rgba=[g.rgba[0],g.rgba[1],g.rgba[2],g.rgba[3]]);g.rgb&&(g.rgb=e.rgba(g.rgb),e.point.rgba=[g.rgb[0],g.rgb[1],g.rgb[2],255]);b=g;e.event("beforeEffect",{layer:n,name:a,params:b});"function"==typeof d.effects[a]?b=d.effects[a](e,b):console.warn("effect not callable: "+
a);void 0===b&&(b=c);e.event("afterEffect",{layer:n,name:a,params:b});!1!==b.store&&h.push([n,a,b]);return e};return e}}})("tgen");((f)=>{f=window[f];f.colormap("blackwhite",()=>[{percent:0,rgba:[0,0,0,255]},{percent:25,rgba:[255,255,255,255]},{percent:50,rgba:[0,0,0,255]},{percent:75,rgba:[255,255,255,255]},{percent:100,rgba:[0,0,0,255]}]);f.colormap("dawn",()=>[{percent:0,rgba:[255,255,192,255]},{percent:25,rgba:[255,255,128,255]},{percent:50,rgba:[255,128,128,255]},{percent:75,rgba:[128,0,128,255]},{percent:100,rgba:[0,0,128,255]}]);f.colormap("dusk",()=>[{percent:0,rgba:[255,255,255,255]},{percent:25,rgba:[255,128,255,255]},
