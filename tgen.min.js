...
void 0==a&&(a=this.background);for(var b=this.size();b;)this.data[b-1]=a[3],this.data[b-2]=a[2],this.data[b-3]=a[1],this.data[b-4]=a[0],b-=this.components};this.pattern=function(a,b){var c=a/b;if(a>=b)return c=Math.floor(c)*b,a-c;if(0>a)return c=Math.ceil(c)*b,c=b-Math.abs(a-c),c>=b&&(c-=b),c};this.offset=function(a,b){a=Math.round(a);b=Math.round(b);if(0>a||a>=this.width)a=this.pattern(a,this.width);if(0>b||b>=this.height)b=this.pattern(b,this.height);return b*this.width*this.components+a*this.components};
...
f){e/=f;d=d-c-(a-b);a=d*e*e*e+(a-b-d)*e*e+(c-a)*e+b;0>a&&(a=0);1<a&&(a=1);return a}}};e.colormap={data:null,size:255,init:function(a,c,f){this.data=null;this.size=void 0==c?b:c;if(void 0==a||null==a)return a;if("object"==typeof a)if("object"==typeof a[0])for(key in a){var k=a[key];k.rgba=e.rgba(k.rgba);a[key]=k}else a=e.randItem(a);if("random"===a){k=e.randInt(1,4);a=[];for(var r=0;r<=k;r++)a[r]={percent:parseInt(r/k*100),rgba:[e.randInt(0,255),e.randInt(0,255),e.randInt(0,255),255]}}"function"==
typeof f&&f(a);"string"==typeof a&&(f=!1,"!"==a.charAt(0)&&(a=a.substring(1),f=!0),"function"==typeof d.colormaps[a]&&(c=d.colormaps[a](c),this.data=this.render(c,f)));"object"==typeof a&&(this.data=this.render(a))},render:function(a,b){for(var c=[],d=0;d<a.length-1;d++)for(var e=a[d],k=a[d+1],f=Math.round(e.percent/100*this.size),r=Math.round(k.percent/100*this.size),h=f;h<=r;h++)c[b?this.size-h:h]=[e.rgba[0]+(h-f)/(r-f)*(k.rgba[0]-e.rgba[0]),e.rgba[1]+(h-f)/(r-f)*(k.rgba[1]-e.rgba[1]),e.rgba[2]+
(h-f)/(r-f)*(k.rgba[2]-e.rgba[2]),e.rgba[3]+(h-f)/(r-f)*(k.rgba[3]-e.rgba[3])];return c},get:function(a,b){if(void 0==a)return this.data;a=e.calc.pingpong(parseInt(a),0,this.size);var c=this.data[a];void 0!==b&&(c[3]=b[3]);return c}};e.wrapx=function(a){return a&b-1};e.wrapy=function(b){return b&a-1};e.point={rgba:[],mixed:[],blend:"opacity",colorize:function(a,b,c){void 0===c&&(c=50);return[a[0]-c/100*(a[0]-b[0]),a[1]-c/100*(a[1]-b[1]),a[2]-c/100*(a[2]-b[2]),b[3]?b[3]:255]},opacity:function(a,b){if(255==
...
this.mixed[3]=Math.max(0,Math.min(255,this.mixed[3]));break;case "pingpong":this.mixed[0]=e.calc.pingpong(this.mixed[0],0,255),this.mixed[1]=e.calc.pingpong(this.mixed[1],0,255),this.mixed[2]=e.calc.pingpong(this.mixed[2],0,255),this.mixed[3]=e.calc.pingpong(this.mixed[3],0,255)}e.texture.set(a,b,this.mixed)},get:function(a,b,c){return e.texture.get(a,b)}};e.walk=function(c){for(var d=0;d<b;d++)for(var f=0;f<a;f++){var k=e.point.get(d,f),k=c(k,d,f);e.point.rgba=k;e.point.set(d,f)}};e.percent=function(a,
b){return parseInt(a/100*b,10)};e.percentX=function(a){return parseInt(a/100*b,10)};e.percentY=function(b){return parseInt(b/100*a,10)};e.percentXY=function(a){return parseInt(a/100*g,10)};e.xysize=function(c,d){if(void 0!=d.elements)var f=d.elements[c].x,k=d.elements[c].y,h=d.elements[c].size;else"random"==d.origin?(f=e.randIntSeed(0,b),k=e.randIntSeed(0,a)):(f=d.origin[0],k=d.origin[1]),h=e.randByArraySeed(d.size);return{x:f,y:k,size:h}};e.toContext=function(c,d){for(var e=c.createImageData(b,a),
f=e.data,k=d.length,h=0;h<k;h+=4)f[h]=d[h],f[h+1]=d[h+1],f[h+2]=d[h+2],f[h+3]=d[h+3];return e};e.toCanvas=function(c){void 0==c&&(c=e.texture.data);var d=document.createElement("canvas");d.width=b;d.height=a;var f=d.getContext("2d");c=this.toContext(f,c);f.putImageData(c,0,0);return d};e.getCanvas=function(a){if(a){var b=this.toCanvas(this.layers[this.layers.length-1]);a(b)}return this};e.getPhases=function(a){if(a){for(var b=[],c=e.layers.length,d=0;d<c;d++)b.push(this.toCanvas(this.layers[d]));
a(b)}return this};e.stat=function(a){h.stop=(new Date).getTime();h.elapsed=(h.stop-h.start)/1E3;a&&a(h);return this};e.history={available:function(){try{return"localStorage"in window&&null!==window.localStorage&&void 0!==window.localStorage}catch(k){return!1}},list:function(){return this.available()?JSON.parse(window.localStorage.getItem(d.config.historyName))||[]:[]},last:function(){return this.available()?d.config.historyList[d.config.historyList.length-1]:[]},get:function(a){return this.available()?
d.config.historyList[a]?d.config.historyList[a]:null:[]},add:function(a){if(d.config.historyLast){if(!this.available())return[];a=e.params(a);d.config.historyList=JSON.parse(window.localStorage.getItem(d.config.historyName))||[];d.config.historyList.length>=d.config.historyLast&&d.config.historyList.shift();d.config.historyList.push(a);window.localStorage.setItem(d.config.historyName,JSON.stringify(d.config.historyList))}}};e.params=function(c){void 0==c&&(c=new Date,c=c.getHours()+":"+c.getMinutes()+
":"+c.getSeconds()+" l"+e.layers.length+" i"+f.length);return{name:c,version:d.version,width:b,height:a,normalize:e.normalize,items:f}};e.render=function(c,f){e.event("beforeRender",c);var h=0;void 0!=c.width&&(b=c.width);void 0!=c.height&&(a=c.height);e.normalize=void 0!=c.normalize?c.normalize:"limitless";l();e.texture=new e.buffer(c.background);1!=f&&e.clear();for(var g in c.items){p=c.items[g][0];var k=c.items[g][1],m=c.items[g][2];"random"==k&&(k=t(e.defaults));"object"==typeof k&&(k=e.randItem(k));
h!=p&&(void 0!=e.layers[p]?e.texture.data=e.layers[p]:e.texture.clear(),h=p);if(void 0!=e[k])e[k](m);else void 0!=d.effects[k]?e["do"](k,m):console.warn("undefined effect: "+k);e.layers[p]=e.texture["export"]()}e.event("afterRender",e.params());e.history.add();return this};e.event=function(a,b){for(var c in d.events[a]){var f=d.events[a][c];"function"==typeof f&&f(e,b)}};e["do"]=function(a,b){var c=b;void 0===b?b=n({},d.defaults[a]):"object"==typeof b&&(b=n(d.defaults[a],b));var h=b;"object"==typeof h.count&&
(h.count=e.randInt(h.count[0],h.count[1]));"random"===h.blend&&(h.blend=t(d.blends));"object"==typeof h.blend&&(h.blend=h.blend[e.randInt(0,h.blend.length-1)]);e.point.blend=void 0!==h.blend?h.blend:"";h.rgba&&(h.rgba=e.rgba(h.rgba),e.point.rgba=[h.rgba[0],h.rgba[1],h.rgba[2],h.rgba[3]]);h.rgb&&(h.rgb=e.rgba(h.rgb),e.point.rgba=[h.rgb[0],h.rgb[1],h.rgb[2],255]);b=h;e.event("beforeEffect",{layer:p,name:a,params:b});"function"==typeof d.effects[a]?b=d.effects[a](e,b):console.warn("effect not callable: "+
a);void 0===b&&(b=c);e.event("afterEffect",{layer:p,name:a,params:b});!1!==b.store&&f.push([p,a,b]);return e};return e}}})("tgen");(function(g){g=window[g];g.colormap("blackwhite",function(){return[{percent:0,rgba:[0,0,0,255]},{percent:25,rgba:[255,255,255,255]},{percent:50,rgba:[0,0,0,255]},{percent:75,rgba:[255,255,255,255]},{percent:100,rgba:[0,0,0,255]}]});g.colormap("dawn",function(){return[{percent:0,rgba:[255,255,192,255]},{percent:25,rgba:[255,255,128,255]},{percent:50,rgba:[255,128,128,255]},{percent:75,rgba:[128,0,128,255]},{percent:100,rgba:[0,0,128,255]}]});g.colormap("dusk",function(){return[{percent:0,rgba:[255,
...
!0);a.size=b.randByArray(a.size);for(var c=0;c<a.count;c++)b.shape.line(b,b.texture.width/2+Math.sin(c/a.freq1s*b.calc.pi)*a.size,b.texture.height/2+Math.cos(c/a.freq1c*b.calc.pi)*a.size,b.texture.width/2+Math.sin(c/a.freq2s*b.calc.pi)*a.size,b.texture.height/2+Math.cos(c/a.freq2c*b.calc.pi)*a.size);return a});g.effect("lines2",{blend:["opacity","lighten","screen"],rgba:"random",type:"vertical",size:[.1,11],count:[4,21],seed:[1,262140]},function(b,a){var c;a.seed=b.randByArray(a.seed);b.calc.randomseed(a.seed);
for(var d=0;d<a.count;d++)c=void 0!=a.elements?a.elements[d]:{size:b.randByArraySeed(a.size,!0),d:b.randRealSeed(.1,100)},"vertical"==a.type?b.shape.rect(b,b.percentX(c.d),0,b.percentX(c.size),b.texture.height):b.shape.rect(b,0,b.percentX(c.d),b.texture.width,b.percentX(c.size));return a});g.effect("subplasma",{seed:[1,262140],size:[3,4],rgba:"random"},function(b,a){a.seed=b.randByArray(a.seed);a.size=b.randByArray(a.size);b.calc.randomseed(a.seed);var c=1<<a.size,d=b.texture.width,f=[],h,g;c>d&&
(c=d);var m=d/c;for(g=0;g<c;g++)for(h=0;h<c;h++)f[h*m+g*m*d]=b.calc.randomseed();for(g=0;g<c;g++)for(h=0;h<d;h++){var e=h&~(m-1),l=g*m*d;f[h+l]=b.calc.interpolate.catmullrom(f[(e-1*m&d-1)+l],f[(e-0*m&d-1)+l],f[(e+1*m&d-1)+l],f[(e+2*m&d-1)+l],h%m,m)}for(g=0;g<d;g++)for(h=0;h<d;h++)e=g&~(m-1),f[h+g*d]=b.calc.interpolate.catmullrom(f[h+(e-1*m&d-1)*d],f[h+(e-0*m&d-1)*d],f[h+(e+1*m&d-1)*d],f[h+(e+2*m&d-1)*d],g%m,m);for(h=0;h<b.texture.width;h++)for(g=0;g<b.texture.height;g++)c=255*f[h+g*d],b.point.rgba=
b.point.colorize(a.rgba,[c,c,c,255]),b.point.set(h,g);return a});g.effect("waves",{blend:"opacity",rgba:"random",level:50,xsines:[1,10],ysines:[1,10]},function(b,a){void 0===a.xsines?a.xsines=b.randInt(1,10):"object"==typeof a.xsines&&(a.xsines=b.randInt(a.xsines[0],a.xsines[1]));void 0===a.ysines?a.ysines=b.randInt(1,10):"object"==typeof a.ysines&&(a.ysines=b.randInt(a.ysines[0],a.ysines[1]));void 0===a.rgba&&(a.rgba=b.rgba([[0,255],[0,255],[0,255],void 0!==a.opacity?a.opacity:255]));for(var c=0;c<
...
a.xchannel=b.randByArray(a.xchannel);a.ychannel=b.randByArray(a.ychannel);a.xlayer=b.randByArray(a.xlayer);a.ylayer=b.randByArray(a.ylayer);for(var c=new b.buffer,d=b.texture.width,f=b.texture.height,h=b.layers[a.xlayer],g=b.layers[a.ylayer],m=0;m<d;m++)for(var e=0;e<f;e++){var l=b.texture.offset(m,e),n=h[l+a.xchannel],q=g[l+a.ychannel],n=0==d%16?b.wrapx(m+(n*a.xamount*d>>16)):m+n*a.xamount*d/(d*d),q=0==f%16?b.wrapy(e+(q*a.yamount*f>>16)):e+q*a.yamount*f/(f*f),q=b.point.get(n,q);c.data[l]=q[0];c.data[l+
1]=q[1];c.data[l+2]=q[2];c.data[l+3]=q[3]}for(d=b.texture.size();d--;)b.texture.data[d]=c.data[d];return a});g.effect("clouds",{blend:"opacity",rgba:"random",seed:[1,262140],roughness:[2,16],colormap:null},function(b,a){a.seed=b.randByArray(a.seed);a.roughness=b.randByArray(a.roughness);var c=b.texture.width,d=b.texture.height,f=[],h=function(a,b,e){0>a&&(a=c+a);a>=c&&(a-=c);0>b&&(b=d+b);b>=d&&(b-=d);return void 0!==e?f[a][b]=e:f[a][b]},g=function(d){return(b.calc.randomseed()-.5)*(d/(c+c))*a.roughness},
m=function(e){var f=e/2;if(1>=f)return a;for(var k=f-f;k<=c+f;k+=f)for(var p=f-f;p<=d+f;p+=f){var l=h(k-f,p-f),q=h(k,p-f),n=h(k-f,p),v=h(k,p),w=k-f/2,x=p-f/2,v=h(w,x,b.calc.normalize1((l+q+n+v)/4+g(e)));h(k-f,x,b.calc.normalize1((l+n+v+h(k-e+f/2,x))/4+g(e)));h(w,p-f,b.calc.normalize1((l+q+v+h(w,p-e+f/2))/4+g(e)))}m(f)};b.calc.randomseed(a.seed);(function(){for(var a=0;a<=c;a++){f[a]=[];for(var b=0;b<=d;b++)f[a][b]=0}})();m(c);b.colormap.init(a.colormap,255,function(b){a.colormap=b});for(var e=0;e<
c;e++)for(var l=0;l<d;l++){var n=parseInt(255*f[e][l],10);b.point.rgba=null!==b.colormap.data?b.colormap.get(n,a.rgba):b.point.colorize(a.rgba,[n,n,n,255]);b.point.set(e,l)}return a});g.effect("colorbar",{type:"horizontal",mirror:!1,colormap:[{percent:0,rgba:[[0,255],[0,255],[0,255],255]},{percent:25,rgba:[[0,255],[0,255],[0,255],255]},{percent:50,rgba:[[0,255],[0,255],[0,255],255]},{percent:75,rgba:[[0,255],[0,255],[0,255],255]},{percent:100,rgba:[[0,255],[0,255],[0,255],255]}]},function(b,a){var c=
b.texture.width,d=b.texture.height;b.colormap.init(a.colormap,"horizontal"==a.type?c:d,function(b){a.colormap=b});if("horizontal"==a.type)for(var f=0;f<c;f++){if(a.mirror)var h=f<c/2?2*f:2*c-2*f;b.point.rgba=b.colormap.get(h);for(var g=0;g<d;g++)b.point.set(f,g)}else for(g=0;g<d;g++)for(a.mirror&&(h=g<d/2?2*g:2*d-2*g),b.point.rgba=b.colormap.get(h),f=0;f<c;f++)b.point.set(f,g);return a});g.effect("checkerboard",{size:[8,8],rgba:"randomalpha"},function(b,a){var c=b.texture.width,d=b.texture.height;
if("object"==typeof a.size)var f=a.size[0]=b.randByArray(a.size[0]),h=a.size[1]=b.randByArray(a.size[1]);else h=f=a.size;for(var g=c/f,m=d/h,e=function(a,e){for(var f=0;f<g;f++)for(var h=0;h<m;h++)f+a<c&&h+e<d&&b.point.set(f+a,h+e)},l=0;l<f;l++)if(0==l%2)for(var n=0;n<h;n++)0==n%2?e(l*g,n*m):e(l*g+g,n*m);return a});g.effect("dots",{blend:"opacity",gridX:[1,32],gridY:[1,32],size:[10,250],seed:[1,262140],rgba:"randomalpha",shape:"rect",dynamic:!0,xsines:[1,12],ysines:[1,12]},function(b,a){a.gridX=b.randByArray(a.gridX);
a.gridY=b.randByArray(a.gridY);a.seed=b.randByArray(a.seed);void 0===a.xsines?a.xsines=b.randInt(1,10):"object"==typeof a.xsines&&(a.xsines=b.randInt(a.xsines[0],a.xsines[1]));void 0===a.ysines?a.ysines=b.randInt(1,10):"object"==typeof a.ysines&&(a.ysines=b.randInt(a.ysines[0],a.ysines[1]));b.calc.randomseed(a.seed);for(var c=b.randByArraySeed(a.size)/100,d=b.texture.width/a.gridX,f=b.texture.height/a.gridY,h=d/2,g=f/2,m=1;m<=a.gridX;m++)for(var e=1;e<=a.gridY;e++){var l=c*(d+f)/2/2,l=l-l/2*Math.sin(m/
a.gridX*a.xsines*2*b.calc.pi)+l/2*Math.sin(e/a.gridY*a.ysines*2*b.calc.pi);switch(a.shape){case "sphere":b.shape.sphere(b,m*d-h,e*f-g,2*l,!0,a.rgba,a.dynamic);break;case "pyramid":b.shape.pyramid(b,m*d-h,e*f-g,l,l,!0,a.rgba,a.dynamic);break;case "rect":b.shape.rect(b,m*d-h,e*f-g,l,l,!0,a.rgba,a.dynamic);break;default:l/=2,b.shape.circle(b,m*d-h,e*f-g,l,!0)}}return a})})("tgen");(function(g){g=window[g];g.effect("opacity",{adjust:128},function(b,a){b.walk(function(b){b[3]=a.adjust;return b});return a});g.effect("vibrance",{adjust:50},function(b,a){var c=-1*a.adjust;b.walk(function(a){var b=Math.max(a[0],a[1],a[2]),d=2*Math.abs(b-(a[0]+a[1]+a[2])/3)/255*c/100;a[0]!==b&&(a[0]+=(b-a[0])*d);a[1]!==b&&(a[1]+=(b-a[1])*d);a[2]!==b&&(a[2]+=(b-a[2])*d);return[a[0],a[1],a[2],a[3]]});return a});g.effect("brightness",{adjust:50,legacy:!0},function(b,a){!0===a.legacy?b.walk(function(b){return[Math.min(b[0]+
...
255,function(b){a.colormap=b});b.walk(function(c){if(b.colormap.data){var d=b.colormap.get((c[0]+c[1]+c[2])/3,a.rgba);d[3]=c[3];return d}return b.point.colorize(c,a.rgba,a.level)});return a});g.effect("invert",{channels:[1,1,1]},function(b,a){b.walk(function(b){return[a.channels[0]?255-b[0]:b[0],a.channels[1]?255-b[1]:b[1],a.channels[2]?255-b[2]:b[2],b[3]]});return a});g.effect("sobel",{type:1},function(b,a){b["do"]("convolute",{store:!1,transparent:!1,weights:1==a.type?[-1,-2,-1,0,0,0,1,2,1]:[-1,
0,1,-2,0,2,-1,0,1]});return a});g.effect("emboss",{type:1},function(b,a){b["do"]("convolute",{store:!1,transparent:!1,weights:1==a.type?[1,1,1,1,.7,-1,-1,-1,-1]:[-2,-1,0,-1,1,1,0,1,2]});return a});g.effect("edgedetect",{type:1},function(b,a){b["do"]("convolute",{store:!1,transparent:!1,weights:1==a.type?[-1,-1,-1,-1,8,-1,-1,-1,-1]:[0,1,0,1,-4,1,0,1,0]});return a});g.effect("sharpen",{type:1},function(b,a){b["do"]("convolute",{store:!1,transparent:!1,weights:1==a.type?[0,-1,0,-1,5,-1,0,-1,0]:[-1,-1,
-1,-1,9,-1,-1,-1,-1]});return a});g.effect("blur",{},function(b,a){b["do"]("convolute",{store:!1,transparent:!1,weights:[1/9,1/9,1/9,1/9,1/9,1/9,1/9,1/9,1/9]});return a});g.effect("sinecolor",{sines:[1,7],channel:[0,2]},function(b,a){a.sines=b.randByArray(a.sines);a.channel=b.randByArray(a.channel);b.walk(function(c){var d=parseInt(255*Math.sin(b.calc.pi/180*c[a.channel]*(255/360)*a.sines));c[a.channel]=Math.abs(d);return c});return a});g.effect("convolute",{blend:"opacity",transparent:!1,weights:[1,
1,1,1,1,1,1,1,1]},function(b,a){if("object"!=typeof a.weights||null==a.weights)return a;var c=new b.buffer;c.clear();for(var d=Math.round(Math.sqrt(a.weights.length)),f=Math.floor(d/2),h=a.transparent?1:0,g=0;g<b.texture.height;g++)for(var m=0;m<b.texture.width;m++){for(var e=0,l=0,n=0,q=0,t=0;t<d;t++)for(var k=0;k<d;k++)var r=a.weights[t*d+k],u=b.texture.get(m+k-f,g+t-f),e=e+u[0]*r,l=l+u[1]*r,n=n+u[2]*r,q=q+u[3];c.set(m,g,[e,l,n,q+h*(255-q)])}for(d=b.texture.size();d--;)b.texture.data[d]=c.data[d];
return a})})("tgen");(function(g){g=window[g];g.shape("rect",function(b,a,c,d,f,h){void 0!==h&&(a-=parseInt(d/2,10),c-=parseInt(f/2,10));for(h=0;h<d;h++)for(var g=0;g<f;g++)b.point.set(a+h,c+g)});g.shape("circle",function(b,a,c,d,f){void 0==f&&(a+=d,c+=d);for(f=-d;f<d;f++)for(var h=Math.round(Math.sqrt(d*d-f*f)),g=-h;g<h;g++)b.point.set(a+f,c+g)});g.shape("line",function(b,a,c,d,f){var h=Math.sqrt((d-a)*(d-a)+(f-c)*(f-c));d=(d-a)/h;f=(f-c)/h;for(var g,m,e=0;e<h;e++)g=a+d*e,m=c+f*e,b.point.set(g,m)});g.shape("sphere",
function(b,a,c,d,f,g,p){void 0==f&&(a+=d,c+=d);for(f=-d;f<d;f++)for(var h=parseInt(Math.sqrt(d*d-f*f),10),e=-h;e<h;e++){var l=Math.min(255,Math.max(0,255-255*Math.sqrt(e*e+f*f)/(d/2)))/255;0<l&&(b.point.rgba=[g[0]*l,g[1]*l,g[2]*l,p?255*l:g[3]],b.point.set(a+f,c+e))}});g.shape("pyramid",function(b,a,c,d,f,g,p,m){var e=parseInt(d/2,10),h=parseInt(f/2,10);1!=g&&(a+=e,c+=h);for(g=-e;g<e;g++)for(var n=-h;n<h;n++){var q=255*(.25-Math.abs(g/d))+255*(.25-Math.abs(n/f));1<q&&(b.point.rgba=[p[0]/255*q,p[1]/
255*q,p[2]/255*q,m?q:p[3]],b.point.set(a+g,c+n))}})})("tgen");
